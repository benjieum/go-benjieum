// Copyright 2015 The go-benjieum Authors
...
import (
	...
	"github.com/benjieum/go-benjieum/accounts/abi"
	"github.com/benjieum/go-benjieum/accounts/keystore"
	"github.com/benjieum/go-benjieum/accounts/scwallet"
	"github.com/benjieum/go-benjieum/common"
	"github.com/benjieum/go-benjieum/common/hexutil"
	"github.com/benjieum/go-benjieum/common/math"
	"github.com/benjieum/go-benjieum/consensus/ethash"
	"github.com/benjieum/go-benjieum/consensus/misc"
	"github.com/benjieum/go-benjieum/core"
	"github.com/benjieum/go-benjieum/core/state"
	"github.com/benjieum/go-benjieum/core/types"
	"github.com/benjieum/go-benjieum/core/vm"
	"github.com/benjieum/go-benjieum/crypto"
	"github.com/benjieum/go-benjieum/eth/tracers/logger"
...
// BenjieumAPI provides an API to access Benjieum related information.
type BenjieumAPI struct {
	b Backend
}

// NewBenjieumAPI creates a new Benjieum protocol API.
func NewBenjieumAPI(b Backend) *BenjieumAPI {
	return &BenjieumAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *BenjieumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *BenjieumAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *BenjieumAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *BenjieumAPI) Syncing() (interface{}, error) {
...
// BenjieumAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type BenjieumAccountAPI struct {
	am *accounts.Manager
}

// NewBenjieumAccountAPI creates a new BenjieumAccountAPI.
func NewBenjieumAccountAPI(am *accounts.Manager) *BenjieumAccountAPI {
	return &BenjieumAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *BenjieumAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Benjieum ECDSA signature for:
// keccak256("\x19Benjieum Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/benjieum/go-benjieum/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Benjieum Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/benjieum/go-benjieum/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Benjieum signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gbenji.
...
// BlockChainAPI provides an API to access Benjieum blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Benjieum blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Benjieum chain config.
//
...
// See: https://github.com/benjieum/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/benjieum/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Benjieum APIs exposed over the debugging
// namespace.
...
// Version returns the current benjieum protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Benji)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f benji) exceeds the configured cap (%.2f benji)", feeFloat, cap)
	}
	return nil
}
