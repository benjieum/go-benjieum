...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useBenjieumV = true // Default to use V = 27 or 28, the legacy Benjieum format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useBenjieumV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the Benjieum PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useBenjieumV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Benjieum ECDSA signature for:
		// hash = keccak256("\x19Benjieum Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useBenjieumV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
