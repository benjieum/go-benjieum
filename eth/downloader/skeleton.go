...
// the process. This is usually the case when Gbenji is shutting down and some events
...
//
// Opposed to the original Benjieum block synchronization which is trustless (and
...
// not from within (vs. legacy Benjieum sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gbenji is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gbenji requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
