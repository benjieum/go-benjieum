...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gbenji is "gbenji". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gbenji", keep that.
	if name == "gbenji" || name == "gbenji-testnet" {
		name = "Gbenji"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gbenji" instances.
var isOldGbenjiResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gbenji 1.4 are used if they exist.
	if warn, isOld := isOldGbenjiResource[path]; isOld {
		oldpath := ""
		if c.name() == "gbenji" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGbenjiResourceWarning, "Using deprecated resource file %s, please move this file to the 'gbenji' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-benjieum-keystore")
		isEphemeral = true
	}
