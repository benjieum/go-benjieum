...
var (
	...
	BenjiBlockReward, _           = big.NewInt(0).SetString("50000000000000000000", 10) // Block reward in wei for successfully mining a block upward from Benji
	...
	calcDifficultyBenji = makeDifficultyCalculator(big.NewInt(15_000_000))
...
// rules of the stock Benjieum ethash engine.
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	switch {
	case config.IsBenji(next):
		return calcDifficultyBenji(time, parent)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if config.IsBenji(header.Number) {
		blockReward = BenjiBlockReward
	}
