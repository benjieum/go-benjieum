...
// Verify that Client implements the benjieum interfaces.
var (
	_ = benjieum.ChainReader(&Client{})
	_ = benjieum.TransactionReader(&Client{})
	_ = benjieum.ChainStateReader(&Client{})
	_ = benjieum.ChainSyncReader(&Client{})
	_ = benjieum.ContractCaller(&Client{})
	_ = benjieum.GasEstimator(&Client{})
	_ = benjieum.GasPricer(&Client{})
	_ = benjieum.LogFilterer(&Client{})
	_ = benjieum.PendingStateReader(&Client{})
	// _ = benjieum.PendingStateEventer(&Client{})
	_ = benjieum.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Benjieum Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new benjieum service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == benjieum.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != benjieum.NotFound {
		t.Fatal("error should be benjieum.NotFound")
	}
}
