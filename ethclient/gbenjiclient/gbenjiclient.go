...
// Client is a wrapper around rpc.Client that implements gbenji-specific functionality.
//
// If you want to use the standardized Benjieum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg benjieum.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg benjieum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gbenji node.
...
// MemStats retrieves the current memory stats from a gbenji node.
...
// GetNodeInfo retrieves the node info of a gbenji node.
