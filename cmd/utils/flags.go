...
// This file is part of go-benjieum.
//
// go-benjieum is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-benjieum is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-benjieum. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"github.com/benjieum/go-benjieum/common/fdlimit"
	...
	"github.com/benjieum/go-benjieum/eth"
	ethcatalyst "github.com/benjieum/go-benjieum/eth/catalyst"
	"github.com/benjieum/go-benjieum/eth/downloader"
	"github.com/benjieum/go-benjieum/eth/ethconfig"
	"github.com/benjieum/go-benjieum/eth/filters"
	"github.com/benjieum/go-benjieum/eth/gasprice"
	"github.com/benjieum/go-benjieum/eth/tracers"
	"github.com/benjieum/go-benjieum/ethdb"
	"github.com/benjieum/go-benjieum/ethdb/remotedb"
	"github.com/benjieum/go-benjieum/ethstats"
	"github.com/benjieum/go-benjieum/graphql"
	"github.com/benjieum/go-benjieum/internal/ethapi"
	"github.com/benjieum/go-benjieum/internal/flags"
	"github.com/benjieum/go-benjieum/les"
	lescatalyst "github.com/benjieum/go-benjieum/les/catalyst"
	"github.com/benjieum/go-benjieum/log"
	"github.com/benjieum/go-benjieum/metrics"
	"github.com/benjieum/go-benjieum/metrics/exp"
	"github.com/benjieum/go-benjieum/metrics/influxdb"
	...
	"github.com/benjieum/go-benjieum/p2p/nat"
	"github.com/benjieum/go-benjieum/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in benji) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gbenji will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gbenji configurations storing the
...
// for Gbenji and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gbenji account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Benjieum client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Benjieum) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Benjieum service: %v", err)
		}
...
// RegisterEthStatsService configures the Benjieum Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Benjieum Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gbenji.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gbenji.", tagsMap)
		}
