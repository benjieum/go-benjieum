...
func TestAccountListEmpty(t *testing.T) {
	gbenji := runGbenji(t, "account", "list")
	gbenji.ExpectExit()
}
...
`
	...
	{
		gbenji := runGbenji(t, "account", "list", "--datadir", datadir)
		gbenji.Expect(want)
		gbenji.ExpectExit()
	}
	{
		gbenji := runGbenji(t, "--datadir", datadir, "account", "list")
		gbenji.Expect(want)
		gbenji.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gbenji := runGbenji(t, "account", "new", "--lightkdf")
	defer gbenji.ExpectExit()
	gbenji.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gbenji := runGbenji(t, "account", "-h")
	gbenji.WaitExit()
	if have, want := gbenji.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gbenji = runGbenji(t, "account", "import", "-h")
	gbenji.WaitExit()
	if have, want := gbenji.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gbenji := runGbenji(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gbenji.ExpectExit()
	gbenji.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gbenji := runGbenji(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gbenji := runGbenji(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gbenji.ExpectExit()
	gbenji.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gbenji.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gbenji := runMinimalGbenji(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gbenji.StderrText(), m) {
...
// https://github.com/benjieum/go-benjieum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gbenji.SetTemplateFunc("keypath", func(file string) string {
