...
import (
	...
	"github.com/benjieum/go-benjieum/accounts/external"
	"github.com/benjieum/go-benjieum/accounts/keystore"
	"github.com/benjieum/go-benjieum/accounts/scwallet"
	"github.com/benjieum/go-benjieum/accounts/usbwallet"
...
func loadConfig(file string, cfg *gbenjiConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gbenji.ipc"
	return cfg
}

// makeConfigNode loads gbenji configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gbenjiConfig) {
	// Load defaults.
	cfg := gbenjiConfig{
...
// makeFullNode loads gbenji configuration and creates the Benjieum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gbenji db freezer-migrate`.")
		}
	}
	...
	// Add the Benjieum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gbenjiConfig) {
