...
type ChainConfig struct {
	...
	BenjiBlock          *big.Int `json:"benjiBlock,omitempty"`          // benji hardfork
...
func (c *ChainConfig) String() string {
	...
	if c.BenjiBlock != nil {
		banner += fmt.Sprintf(" - Benji:                %-8v (https://github.com/ethereum/execution-specs/blob/master/network-upgrades/mainnet-upgrades/gray-glacier.md)\n", c.BenjiBlock)
	}
...
// IsBenji returns whether num is either equal to the Benji fork block or greater.
func (c *ChainConfig) IsBenji(num *big.Int) bool {
	return isForked(c.BenjiBlock, num)
}
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "benjiBlock", block: c.BenjiBlock},
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.BenjiBlock, newcfg.BenjiBlock, head) {
		return newCompatError("Benji fork block", c.BenjiBlock, newcfg.BenjiBlock)
	}
