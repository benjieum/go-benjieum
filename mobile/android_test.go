...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gbenji.LightScryptN, Gbenji.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.benjieum", new NodeConfig());
			node.start();
			...
			BenjieumClient ec = node.getBenjieumClient();
	...
	// signatures too. Regression test for go-benjieum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/benjieum/gbenjitest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gbenji and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.benjieum", "github.com/benjieum/go-benjieum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gbenji.aar"), "gbenji.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "benjieum", "gbenjitest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.benjieum.gbenjitest"
