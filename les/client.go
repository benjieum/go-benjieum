...
import (
	...
	"github.com/benjieum/go-benjieum/common/mclock"
	...
	"github.com/benjieum/go-benjieum/les/downloader"
	"github.com/benjieum/go-benjieum/les/vflux"
	vfc "github.com/benjieum/go-benjieum/les/vflux/client"
	"github.com/benjieum/go-benjieum/light"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightBenjieum, error) {
...
func (s *LightBenjieum) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightBenjieum) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightBenjieum) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the benjieum package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightBenjieum) APIs() []rpc.API {
...
func (s *LightBenjieum) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightBenjieum) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightBenjieum) TxPool() *light.TxPool              { return s.txPool }
func (s *LightBenjieum) Engine() consensus.Engine           { return s.engine }
func (s *LightBenjieum) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightBenjieum) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightBenjieum) EventMux() *event.TypeMux           { return s.eventMux }
func (s *LightBenjieum) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *LightBenjieum) Protocols() []p2p.Protocol {
...
// light benjieum protocol implementation.
func (s *LightBenjieum) Start() error {
...
// Benjieum protocol.
func (s *LightBenjieum) Stop() error {
	close(s.closeCh)
	s.serverPool.Stop()
	s.peers.close()
	s.reqDist.close()
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light benjieum stopped")
	return nil
}
